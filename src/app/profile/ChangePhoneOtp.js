...
const ChangePhoneOTP = (props) => {
  ...
  async function updatePhoneFunction() {
    try {
      console.log('Regenerate OTP for: ', props.user);
      const token = localStorage.getItem('srmToken');

      const response = await ProfileService.updatePhone(
        token,
        props.editableId,
        {
  ...
  async function postPhoneFunction() {
    try {
      const token = localStorage.getItem('srmToken');

      const response = await ProfileService.postPhone(token, {
        user_id: props.userId,
        phone_number: props.user,
        is_primary: 'false',
      });
