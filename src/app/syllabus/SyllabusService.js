...
class SyllabusService {
	getSyllabus(token) {
		return axios.get(`${BACKEND_API_URL}/syllabus`, {
			headers: {
	...
	getSyllabusBySubject(token, subjectID) {
		return axios.get(`${BACKEND_API_URL}/syllabus?subject_id=` + subjectID, {
			headers: {
				accept: 'application/json',
				Authorization: `Bearer ${token}`,
			},
	...
	deleteChapter(token, id) {
		return axios.delete(`${BACKEND_API_URL}/syllabus/${id}`, {
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${token}`,
			},
