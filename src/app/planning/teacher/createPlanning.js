...
const CreatePlanning = () => {
	...
	const fetchPlanning = async (planning_id) => {
		const response = await PlanningService.getPlanningID(token, planning_id)
	...
	const saveChapter = async () => {
		...
		try {
			const response = await PlanningService.savePlanning(token, params.id, {
				class_id: classID,
				term_id: termID !== 'Term' ? termID : null,
				subject_id: subjectID !== 'Subject'  ? subjectID : null,
				month_id: monthID !== 'Month' ? monthID : null,
				...
				<div className={classes.container}>
					...
					<div className={classes.div}>
						...
						<FormControl className={classes.fieldStyle}>
							<Dropdown
								data={monthMenu}
								loading={monthMenuLoading}
								onChange={handleMonthMenuChange}
								makeDisable={true}
								initialValue='Month'
								value={monthID}
							/>
