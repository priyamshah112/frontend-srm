...
const TimetableHomework = (props) => {
  ...
  const [SubjectState, setClassState] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchDraft = async () => {
      try {
        const response = await TimetableService.fetchDraftHomework(
          { id },
          props.token
        );
    ...
    const fetchSubject = async () => {
      try {
        ...
        const response = await TimetableService.fetchAllSubject(token);
  ...
  const saveDetails = async (isBack) => {
    try {
      ...
      const response = await TimetableService.saveHomework(
        { id },
        {
          title,
...
const handleClickShowPassword = () => {
  ...
  const publishData = async (publisedDate, status) => {
    try {
      ...
      const response = await TimetableService.publishHomework(
        { id },
        {
          title: title,
          main_content: description,
    ...
    <>
      <div>
        <form className={classes.formStyle} onSubmit={submitForm}>
          ...
          <Box className={`${classes.margin} ${classes.sideMargins}`}>
            <Grid container className={classes.selectoption}>
              <FormControl className={ `${classes.titleText} ${classes.selectoption}`}>
                <NativeSelect
                  inputProps={{
                    name: 'subject',
                    id: 'subject',
          ...
          <Box className={classes.sideMarginright}>
            <div className={`${classes.reminders} ${classes.margin} ${classes.sideMargins}`} >
                <FormControlLabel
                control={
                          <Checkbox
            ...
            <div className={`${classes.preview} ${classes.margin} ${classes.sideMargins}`}>
          ...
          <Box className={` ${classes.sideMargins}`}>
            ...
            >
              <Grid item sm={12} xs={12}  className={classes.publishBtns}>
                
                <Button
                  id='publishBtn'
                  variant='contained'
                  className={`${classes.fieldStyle} ${'publishBtn'} ${'buttonstyle'}`}
                  color='primary'
              ...
              <Grid item sm={12} xs={12} className={classes.textAlignLeft}>
                <Button
                  id='publishLaterBtn'
                  variant="raised"
...
export default connect(mapStateToProps)(TimetableHomework);
