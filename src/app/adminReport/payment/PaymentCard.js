...
function PaymentCard(props) {
  ...
  const { data, loading, reportLoading } = props
    ...
    <>
      ...
      {!data.length && !loading ? (
        <Typography className={classes.message}>
          No payment report available yet!
        </Typography>
      ) : (
        ''
      )}
            ...
            <CardContent className={classes.cardContent}>
              <Grid container>
                <Grid item xs={8}>
                  <span
                    onClick={() =>
                      history.push(`/payment-report/details/${item.id}`)
                    }
                  >
                    {item.title ? (
...
const mapStateToProps = (state) => {
  const { deleteReportLoading, downloadReportLoading } = state.Report
