...
function Attendance(props) {
  ...
  const onSuccess = (result) => {
    history.push(`/attendance-report/edit/${result.data.id}`)
  }
  const handleNew = () => {
  ...
  const handleSuccess = (result) => {
    ...
    if (result) {
      setAttendanceData([...attendanceData, ...result.data.attendance.data])
      setInfo(result.data.attendance)
    }
  }
  const fail = (e) => {
    setLoader(false)
  }
  const fetchAttendance = (currentPage) => {
    props.getReport('attendance', currentPage, handleSuccess, fail)
  }
  useEffect(() => {
    fetchAttendance(1)
  }, [])
...
export default connect(mapStateToProps, { postReport, getReport })(Attendance)
