...
function NewsDetails(props) {
  ...
  const fetchNewsDetails = (currentPage) => {
    setLoader(true)
    props.getReportById(id, 'news', currentPage, onSuccess)
  }
  useEffect(() => {
    fetchNewsDetails(1)
  }, [])
  ...
  const downloadReport = () => {
    ...
    Axios({
      url: `${downloadReportEndpoint}/${id}?get_type=news&get_report=sheet`,
      method: 'GET',
      params,
      responseType: 'blob',
      ...
      .then((response) => {
        ...
        link.href = url
        link.setAttribute('download', 'news.xlsx')
    ...
    <>
      <div onScroll={handleLoadMore} className={classes.sectionContainer}>
        ...
        <Card className={classes.card}>
          ...
          <NewsTable report={report} loading={loading} />
          {loading ? (
