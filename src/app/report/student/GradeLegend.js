...
const GradeLegend = (props) => {
	...
	useEffect(() => {
		...
		if (testData && testData.class_id) {
			async function getGrades() {
				try {
					const response = await ReportService.getGrades(
						token,
						testData.class_id
					)
				...
				{gradeData.map((obj, key) => {
					return (
						<div className={classes.fillGrade} key={key}>
							...
							>
								<HighlightOffOutlinedIcon className={classes.deleteBtn} />
							</Button>
							<Button
								variant='contained'
								disableElevation
							...
							>
								<AddCircleOutlineOutlinedIcon className={classes.addBtn} />
							</Button>
						</div>
					)
				})}
	...
	const renderEmptyGrade = () => {
		return (
			<div>
				{!isLoading && (
					<div className={classes.emptyGradeMessage}>
